/**
 * balance-check.test.ts — Headless balance audit for MVP-02Y.
 * Runs 1000 ticks across 3 seeds, prints survival metrics.
 *
 * Usage: npx vitest run packages/core-sim/__tests__/balance-check.test.ts
 */
...
describe("MVP-02Y Balance Audit", () => {
  it("runs 1000 ticks across 3 seeds and prints metrics", () => {
    ...
    for (const seed of SEEDS) {
      ...
      results.push(r);
      console.log(`Seed ${seed}: ${elapsed}ms — Alive: ${r.aliveAtEnd}/5, Deaths: ${r.totalDeaths}, First death: tick ${r.firstDeathTick ?? "none"}`);
    }
    ...
    for (const r of results) {
      ...
      console.log(`  Hunger@200: [${r.hungerAt200.map(h => Math.round(h)).join(", ")}]`);
    }
    ...
    const thirstDeaths = results.reduce((s, r) => s + (r.deathCauses["thirst"] ?? r.deathCauses["dehydration"] ?? 0), 0);
    ...
    console.log(`  Thirst deaths total:      ${thirstDeaths}`);
    console.log(`  Exposure deaths total:    ${exposureDeaths}`);
    console.log(`  Avg move rejections:      ${avg("totalMoveRejections")} (terrain cooldown)`);
